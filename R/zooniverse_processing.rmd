...
Compare most-confident class for each image with expert level. 

```{r}
library(caret)

maxLabs <- select(labelData,subject_id,Expert.Label) %>%
  rename(image=subject_id)%>%
  merge(maxLabs) %>%
  mutate(Agree = Expert.Label==maxLabel)

# confusionMatrix(as.factor(maxLabs$Expert.Label), as.factor(maxLabs$maxLabel), positive = "True")

LabSummary<-maxLabs %>%
  group_by(Expert.Label) %>%
  summarize(Correct = sum(Agree==TRUE)/n())

ggplot(LabSummary,aes(Expert.Label,Correct,fill=Expert.Label))+geom_col()+
